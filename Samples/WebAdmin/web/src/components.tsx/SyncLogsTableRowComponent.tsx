import { Box, Collapse, Fab, IconButton, styled, Table, TableBody, TableCell, tableCellClasses, TableHead, TableRow } from "@mui/material";
  ...
  return (
    <>
      <StyledTableRow key={props.row.sessionId}>
        ...
        <StyledTableCell >
          {props.row.sessionId.substring(0, 9)}...
        </StyledTableCell>
        <StyledTableCell>{props.row.clientScopeId.substring(0, 9)}...</StyledTableCell>
          ...
          <Collapse in={openHistory} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              <Table size="small" aria-label="purchases">
                ...
                <TableBody>
                  {props.row.details.filter(sld => sld.totalChangesApplied > 0 || sld.totalChangesSelected > 0 || sld.totalResolvedConflicts > 0).map((historyRow) => (
