...
//    public class SyncMemoryProvider : RemoteOrchestrator, IDisposable
//    {
...
//                    httpMessageResponse = await this.EnsureScopeAsync(httpMessage).ConfigureAwait(false);
//                    break;
...
//        private async Task<HttpMessage> EnsureScopeAsync(HttpMessage httpMessage)
//        {
...
//            var (syncContext, lstScopes) = await this.EnsureScopeAsync(
//                httpMessage.SyncContext, this.sc).ConfigureAwait(false);
