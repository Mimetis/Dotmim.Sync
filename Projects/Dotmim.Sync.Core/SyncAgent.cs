...
//            this.RemoteProvider.SetSchema(c => c.ScopeName = scopeName);
...
//                    c.Add(tbl);
//            });
//            this.RemoteProvider.SetSchema(c =>
//            {
//                foreach (var tbl in tables)
//                    c.Add(tbl);
//            });
...
//                    new MessageBeginSession { Schema = this.LocalProvider.Configuration }).ConfigureAwait(false);
...
//                // We want to get, from local provider, all rows that are not last updated from the server
//                // That's why we need to know the scope info ID from server
