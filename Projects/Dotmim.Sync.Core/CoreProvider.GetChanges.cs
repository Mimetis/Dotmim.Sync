                    ...
                    try
                    {
                        ...
                        foreach (var tableDescription in configTables.Tables)
                        {
                            ...
                            if (this.CanBeServerProvider && context.Parameters != null && context.Parameters.Count > 0 && filters != null && filters.Count > 0)
                            {
                                ...
                                {
                                    var tableFilter = tableDescription.DmSet.Tables[filter.FilterTable.TableName.ObjectNameNormalized];

                                    var hierarchy = tableDescription.GetParentsTo(tableFilter);

                                    if (hierarchy.Count > 0 || tableDescription.TableName.ToLowerInvariant() == filter.FilterTable.TableName.ObjectNameNormalized.ToLowerInvariant())
                                    {
                                        filtersName.Add(filter.InParameter.ColumnName);
                                    }
