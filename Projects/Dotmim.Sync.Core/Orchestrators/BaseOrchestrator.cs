...
namespace Dotmim.Sync
{
    public abstract class BaseOrchestrator
    {
        ...
        /// <param name="provision">Provision enumeration to determine which components to apply</param>
        /// <returns>Full schema with table and columns properties</returns>
        public virtual async Task<SyncSet> MigrationAsync(SyncSet schema, SyncSetup oldSetup, SyncSetup newSetup, CancellationToken cancellationToken = default, IProgress<ProgressArgs> progress = null)
        {
            if (!this.StartTime.HasValue)
                this.StartTime = DateTime.UtcNow;
            ...
            using (var connection = this.Provider.CreateConnection())
            {
                try
                {
                    ...
                    using (var transaction = connection.BeginTransaction())
                    {
                        ...
                        {
                            var tableBuilder = this.Provider.GetTableBuilder(new SyncTable(migrationTable.Table.TableName, migrationTable.Table.SchemaName), oldSetup);

                            // set if the builder supports creating the bulk operations proc stock
                            tableBuilder.UseBulkProcedures = this.Provider.SupportBulkOperations;
                            tableBuilder.UseChangeTracking = this.Provider.UseChangeTracking;
                        ...
                        foreach (var migrationTable in migrationResults.Tables)
                        {
                            var tableBuilder = this.Provider.GetTableBuilder(schema.Tables[migrationTable.Table.TableName, migrationTable.Table.SchemaName], newSetup);
                            ...
                            if (migrationTable.TrackingTable == MigrationAction.CreateOrRecreate)
                            {
                                await tableBuilder.DropTrackingTableAsync(connection, transaction);
                        ...
                        // Only create table if we are on local orchestrator
                        ...
                        if (this is LocalOrchestrator)
                        {
                            ...
                            localScope.Setup = JsonConvert.SerializeObject(newSetup);
                            localScope.Schema = JsonConvert.SerializeObject(schema);
                        ...
                        else
                        {
                            ...
                            remoteScope.Setup = JsonConvert.SerializeObject(newSetup);
                            remoteScope.Schema = JsonConvert.SerializeObject(schema);
